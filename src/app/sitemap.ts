import { MetadataRoute } from "next";

export default function sitemap(): MetadataRoute.Sitemap {
  const base = process.env.NEXT_PUBLIC_APP_URL ?? "https://pixzastudio.com";
  const now = new Date();

  return [
    { url: `${base}/landing`,                    lastModified: now, changeFrequency: "weekly",  priority: 1.0 },
    { url: `${base}/tools`,                      lastModified: now, changeFrequency: "weekly",  priority: 0.9 },
    { url: `${base}/tools/background-remover`,   lastModified: now, changeFrequency: "monthly", priority: 0.9 },
    { url: `${base}/tools/ai-background`,        lastModified: now, changeFrequency: "monthly", priority: 0.8 },
    { url: `${base}/tools/object-remover`,       lastModified: now, changeFrequency: "monthly", priority: 0.8 },
    { url: `${base}/tools/image-upscaler`,       lastModified: now, changeFrequency: "monthly", priority: 0.8 },
    { url: `${base}/batch`,                      lastModified: now, changeFrequency: "weekly",  priority: 0.8 },
    { url: `${base}/blog`,                       lastModified: now, changeFrequency: "daily",   priority: 0.8 },
    { url: `${base}/examples`,                   lastModified: now, changeFrequency: "weekly",  priority: 0.7 },
    { url: `${base}/contact`,                    lastModified: now, changeFrequency: "monthly", priority: 0.5 },
    { url: `${base}/privacy`,                    lastModified: now, changeFrequency: "monthly", priority: 0.3 },
    { url: `${base}/terms`,                      lastModified: now, changeFrequency: "monthly", priority: 0.3 },
  ];
}
