...
async function cfPost(model: string, body: unknown): Promise<Response> {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const rembgRes = await cfPost("@cf/inspyrenet/rembg", { image: toUint8Array(imageBase64) });
    ...
    // Step 2: Generate background (text-to-image — no image input)
    const bgRes = await cfPost("@cf/black-forest-labs/flux-1-schnell", {
