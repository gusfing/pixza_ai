...
export async function POST(req: NextRequest) {
  ...
  try {
    const res = await cfFlux2(CF_ACCOUNT_ID, CF_API_TOKEN, FLUX2_DEV,
      { prompt: "remove the background completely, make background pure white, keep the subject perfectly intact, product photography style, clean white background", width: "1024", height: "1024", steps: "20" },
