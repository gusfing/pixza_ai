...
`;
    ...
    // On macOS, osascript may fail if there's no window server access
    // (e.g. running under a different user session or via SSH)
    const errMsg = error instanceof Error ? error.message : "Failed to open dialog";
    const isDisplayError = errMsg.includes("No such file") || errMsg.includes("not allowed") || errMsg.includes("Application can't be opened") || errMsg.includes("(-1712)") || errMsg.includes("(-600)");

    return NextResponse.json(
      {
        success: false,
        error: isDisplayError
          ? "Cannot open folder picker: the app needs to be run from a Mac login session (not SSH). Please type the path manually."
          : errMsg,
      },
      { status: 500 }
    );
