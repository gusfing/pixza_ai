...
const CLOUDFLARE_MODELS: ProviderModel[] = [
  ...
  },
  {
    ...
    name: "SDXL Lightning",
    description: "Ultra-fast text-to-image generation from ByteDance via Cloudflare. Generates images in 1-4 steps.",
    provider: "cloudflare",
    capabilities: ["text-to-image"],
    isFree: true,
    coverImage: undefined,
  },
  {
    ...
    description: "Artistic and versatile image model optimized with Latent Consistency Models (LCM) via Cloudflare.",
    provider: "cloudflare",
    capabilities: ["text-to-image"],
    isFree: true,
    coverImage: undefined,
...
export async function GET(
  ...
  if (providerFilter) {
    if (providerFilter === "gemini") {
      ...
      if (kieKey) {
        ...
        return NextResponse.json<ModelsErrorResponse>(
          {
            success: false,
            error: "Kie API key required. Platform administrator must configure KIE_API_KEY.",
          },
          { status: 400 }
        );
      }
      ...
      if (wavespeedKey) {
        ...
        return NextResponse.json<ModelsErrorResponse>(
          {
            success: false,
            error: "WaveSpeed API key required. Platform administrator must configure WAVESPEED_API_KEY.",
          },
          { status: 400 }
        );
      }
      ...
      if (cloudflareKey && cloudflareAccountId) {
        ...
        return NextResponse.json<ModelsErrorResponse>(
          {
            success: false,
            error: "Cloudflare credentials required. Platform administrator must configure CLOUDFLARE_ACCOUNT_ID and CLOUDFLARE_API_TOKEN.",
          },
          { status: 400 }
        );
      }
    ...
    includeCloudflare = (cloudflareKey && cloudflareAccountId) ? true : false;
    if (wavespeedKey) {
    ...
    return NextResponse.json<ModelsErrorResponse>(
      {
        success: false,
        error:
          "No external providers are currently configured by the platform administrator.",
      },
      { status: 500 }
    );
  }
  ...
  if (includeCloudflare) {
    ...
    allModels.push(...cfModels);
    providerResults["cloudflare"] = {
      success: true,
      count: cfModels.length,
      cached: true,
    };
    anyFromCache = true;
  }
