...
export async function GET(
  ...
  try {
    ...
    if (provider === "cloudflare") {
      const isImg2Img = decodedModelId.includes("img2img") || decodedModelId.includes("base-1.0");
      result = {
        parameters: [
        ...
        inputs: [
          { name: "prompt", type: "text", required: true, label: "Prompt" },
          ...(isImg2Img ? [{ name: "image_input", type: "image", required: false, label: "Base Image", isArray: true }] as ModelInput[] : [])
        ]
      };
