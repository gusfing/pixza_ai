    ...
    <div className="fixed inset-0 z-[100] bg-white flex flex-col">
      ...
      <div className="h-14 bg-white flex items-center justify-between px-4 border-b border-gray-200">
        <div className="flex items-center gap-1.5">
          ...
          <button onClick={undo} className="px-3 py-1.5 text-xs text-neutral-500 hover:text-white">Undo</button>
          <button onClick={redo} className="px-3 py-1.5 text-xs text-neutral-500 hover:text-white">Redo</button>

          <div className="w-px h-6 bg-gray-200 mx-3" />

          <button onClick={clearAnnotations} className="px-3 py-1.5 text-xs text-neutral-500 hover:text-red-400">Clear</button>
        </div>

        <div className="flex items-center gap-3">
          <button onClick={closeModal} className="px-4 py-1.5 text-xs font-medium text-neutral-500 hover:text-white">
            Cancel
          </button>
      ...
      {/* Bottom Options Bar */}
      <div className="h-14 bg-white flex items-center justify-center gap-6 px-4 border-t border-gray-200">
        {/* Colors */}
          ...
          {STROKE_WIDTHS.map((width) => (
            <button
              ...
              className={`w-8 h-8 rounded flex items-center justify-center transition-colors ${
                toolOptions.strokeWidth === width ? "bg-gray-200" : "hover:bg-gray-100"
              }`}
            >
        ...
        <button
          ...
          className={`px-3 py-1.5 text-[10px] uppercase tracking-wide rounded transition-colors ${
            toolOptions.fillColor ? "bg-gray-200 text-white" : "text-neutral-500 hover:text-white"
          }`}
        >
          Fill
        </button>
        ...
        <div className="flex items-center gap-2 ml-auto">
          <button onClick={() => setScale(Math.max(scale - 0.1, 0.1))} className="w-7 h-7 rounded text-neutral-500 hover:text-white text-sm">-</button>
          <span className="text-[10px] text-neutral-500 w-10 text-center">{Math.round(scale * 100)}%</span>
          <button onClick={() => setScale(Math.min(scale + 0.1, 5))} className="w-7 h-7 rounded text-neutral-500 hover:text-white text-sm">+</button>
        </div>
      </div>
