...
export function ChatPanel({ isOpen, onClose, onBuildWorkflow, isBuildingWorkflow = false, onApplyEdits, workflowState, selectedNodeIds }: ChatPanelProps) {
  ...
  return (
    <div className="fixed top-14 bottom-[200px] right-4 w-[360px] flex flex-col rounded-xl overflow-hidden z-40 animate-slide-up"
      style={{
        background: "rgba(8,8,10,0.92)",
        backdropFilter: "blur(28px) saturate(200%)",
        WebkitBackdropFilter: "blur(28px) saturate(200%)",
        border: "1px solid rgba(146,220,229,0.12)",
        boxShadow: "0 24px 64px rgba(0,0,0,0.7), 0 0 0 1px rgba(238,229,233,0.03) inset",
      }}
    >
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-[#92dce5]/8">
        <div className="flex items-center gap-2">
          <h3 className="text-sm font-semibold text-[#eee5e9]/80">Assistant</h3>
          <span className="px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-widest rounded-md bg-[#d64933]/15 text-[#d64933] border border-[#d64933]/20">Beta</span>
        </div>
        <div className="flex items-center gap-1">
          {messages.length > 0 && (
            <button onClick={() => setMessages([])} className="btn-icon w-7 h-7" aria-label="Clear chat" title="Clear chat">
      ...
      >
        ...
        {messages.length === 0 && !errorMessage && (
          <div className="flex flex-col items-center justify-center h-full text-center px-6 py-12 gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#92dce5]/10 border border-[#92dce5]/20 flex items-center justify-center">
              <svg className="w-5 h-5 text-[#92dce5]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09z" />
              </svg>
            </div>
            <p className="text-sm text-[#eee5e9]/50">Ask me anything about your workflow</p>
            <p className="text-xs text-[#7c7c7c]">"How do I create product photos with different backgrounds?"</p>
          </div>
        )}
              ...
              <div
                className={`max-w-[85%] rounded-xl px-3 py-2 text-sm ${
                  message.role === "user"
                    ...
                    : "bg-white/5 border border-[#92dce5]/10 text-[#eee5e9]/85"
                }`}
              >
                {message.role === "user" ? (
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="bg-white/5 border border-[#92dce5]/8 rounded-xl px-3 py-2.5">
              <div className="flex space-x-1">
                <div className="w-2 h-2 bg-[#92dce5] rounded-full animate-bounce" style={{ animationDelay: "0ms" }} />
                <div className="w-2 h-2 bg-[#92dce5] rounded-full animate-bounce" style={{ animationDelay: "150ms" }} />
                <div className="w-2 h-2 bg-[#92dce5] rounded-full animate-bounce" style={{ animationDelay: "300ms" }} />
              </div>
            </div>
          </div>
        )}
      ...
      {/* Input area */}
      <div className="border-t border-[#92dce5]/8">
        {/* Selection focus chip */}
        {selectedNodeIds && selectedNodeIds.length > 0 && !chipDismissed && (
          <div className="px-3 pt-3">
            <div className="flex items-center gap-2 bg-[#92dce5]/5 border border-[#92dce5]/12 rounded-lg px-3 py-1.5 text-xs text-[#eee5e9]/60">
