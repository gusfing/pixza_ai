...
interface WorkflowStore {
  ...
  saveToDb: (name?: string, description?: string) => Promise<boolean>;
  loadWorkflowsFromDb: () => Promise<{ id: string; name: string; updatedAt: string }[]>;
  loadWorkflowFromDb: (id: string) => Promise<boolean>;
  ...
  refreshModelPricing: () => Promise<void>;
...
const workflowStoreImpl: StateCreator<WorkflowStore> = (set, get) => ({
  ...
  // Model pricing initial state
  modelPricingMap: new Map<string, ModelPricing>(),
  ...
  saveToFile: async () => {
    ...
    // If we have a DB-backed workflow but no local path, use saveToDb
    if (workflowId && !saveDirectoryPath) {
      return get().saveToDb(workflowName || undefined);
    }
      ...
      if (result.success) {
        ...
        if (useExternalImageStorage && workflow.nodes !== currentNodes) {
          const nodesWithRefs = currentNodes.map((node, index) => {
            ...
            if (!externalizedNode || node.id !== externalizedNode.id) return node;
            const mergedData = { ...node.data } as Record<string, unknown>;
            const extData = externalizedNode.data as Record<string, unknown>;
            if (extData.imageRef) mergedData.imageRef = extData.imageRef;
            if (extData.sourceImageRef) mergedData.sourceImageRef = extData.sourceImageRef;
            if (extData.outputImageRef) mergedData.outputImageRef = extData.outputImageRef;
            if (extData.inputImageRefs) mergedData.inputImageRefs = extData.inputImageRefs;
            return { ...node, data: mergedData as WorkflowNodeData } as WorkflowNode;
          });
          set({ nodes: nodesWithRefs, lastSavedAt: timestamp, hasUnsavedChanges: false, imageRefBasePath: saveDirectoryPath });
        } else {
          set({ lastSavedAt: timestamp, hasUnsavedChanges: false, imageRefBasePath: useExternalImageStorage ? saveDirectoryPath : null });
        }
        saveSaveConfig({ workflowId, name: workflowName, directoryPath: saveDirectoryPath, generationsPath: get().generationsPath, lastSavedAt: timestamp, useExternalImageStorage });

        // Dual Save: If user is logged in, also update DB
        get().saveToDb().catch(() => {});

        return true;
      } else {
        useToast.getState().show(`Save failed: ${result.error}`, "error");
        return false;
      }
    } catch (error) {
      useToast.getState().show(`Save failed: ${error instanceof Error ? error.message : "Unknown error"}`, "error");
      return false;
    } finally {
      set({ isSaving: false });
    }
  ...
  saveToDb: async (name?: string, description?: string) => {
    const { nodes, edges, edgeStyle, groups, workflowId, workflowName } = get();
    ...
    try {
      ...
      const res = await fetch("/api/workflow/db", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          id: workflowId,
      ...
      if (result.success) {
        if (!workflowId) {
          set({ workflowId: result.workflow.id });
        }
        return true;
      }
      return false;
  ...
  loadWorkflowFromDb: async (id: string) => {
    try {
      const res = await fetch(`/api/workflow/db/${id}`);
      const result = await res.json();
      if (result.success) {
        const wf = result.workflow.data as WorkflowFile;
        // Inject DB ID back into workflow object
        wf.id = result.workflow.id;
        wf.name = result.workflow.name;
        await get().loadWorkflow(wf);
        return true;
      }
      return false;
    } catch {
